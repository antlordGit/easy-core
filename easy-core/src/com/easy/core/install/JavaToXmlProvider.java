    ...
    protected void collectNavigationMarkers(@NotNull PsiElement element, @NotNull Collection<? super RelatedItemLineMarkerInfo<?>> result) {
        try {
            if (element instanceof PsiMethod) {
                ...
                List<VirtualFile> allFilenames = (List<VirtualFile>) FilenameIndex.getAllFilesByExt(element.getProject(), "xml", GlobalSearchScope.projectScope(element.getProject()));
                List<PsiFile> psiFileList = Lists.newArrayList();
                for (VirtualFile file : allFilenames) {
                    PsiFile[] array = FilenameIndex.getFilesByName(element.getProject(), file.getName(), GlobalSearchScope.projectScope(element.getProject()));
                    if (array.length > 0) {
